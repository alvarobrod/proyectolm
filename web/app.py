	...
	if request.method == 'GET':
		...
		if titulo_form != '':
			...
			if r.status_code == 200:
				js = r.json()
				dic_res = {'titulo': js['results'][0]['title'], 'fecha': funciones.formathora(js['results'][0]['release_date']), 'generos': funciones.genero(js['results'][0]['genre_ids']), 'sinopsis': js['results'][0]['overview'], 'rating': js['results'][0]['vote_average'], 'votos': js['results'][0]['vote_count']}
